            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="row">
                        <div class="col-lg-3">
                            <div class="type">
                                <h2>{{ $donation->bloodgroup }}</h2>
                            </div>
                        </div>
                        <div class="data col-lg-6">
                            <h4>Name: {{ $donation->name }}</h4>
                            <h4>Phone: {{ $donation->phone }}</h4>
                            <h4>City: {{ $donation->city }}</h4>
                        </div>
                        <div class="col-lg-3">
                            <div class="type">
                                
                                <form id="send-request-form" action="{{ route('requests.send') }}" method="post">
                                    @csrf
                                    <input type="hidden" name="donor_id" value="{{ $donation->id }}">
                                    
                                        <a href="{{ route('requests.sendForm', ['donationId' => $donation->id]) }}" class="btn btn-primary" style="width: 200px; margin-top:44px;padding: 10px; margin-right: 10px; background-color:rgb(189, 189, 189); border-color:rgb(189, 189, 189);font-size:20px ;color: darkred">
                                            Sent Request
                                        </a>
                                </form>
